...
const units: { value: string; label: string; toLiters: number }[] = [
  { value: 'liter', label: 'Liter (L)', toLiters: 1 },
  { value: 'milliliter', label: 'Milliliter (mL)', toLiters: 0.001 },
  { value: 'gallon_us', label: 'Gallon (US)', toLiters: 3.785411784 },
  { value: 'gallon_uk', label: 'Gallon (UK)', toLiters: 4.54609 },
  { value: 'cup', label: 'Cup (US)', toLiters: 0.2365882365 },
  { value: 'tablespoon', label: 'Tablespoon (US)', toLiters: 0.01478676478125 },
  { value: 'teaspoon', label: 'Teaspoon (US)', toLiters: 0.00492892159375 },
  { value: 'fluid_ounce', label: 'Fluid Ounce (US)', toLiters: 0.0295735295625 },
  { value: 'barrel', label: 'Barrel (oil)', toLiters: 158.987294928 },
]
...
export default function VolumeConverterTool() {
  ...
  const result = useMemo(() => {
    ...
    const liters = val * from.toLiters
    const converted = liters / to.toLiters
  ...
  return (
    ...
    >
      ...
      {result && (() => {
        const from = units.find(u => u.value === fromUnit)!
        const to = units.find(u => u.value === toUnit)!
        const factor = from.toLiters / to.toLiters
        return (
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {units.map((u) => {
              ...
              const val = parseFloat(input) * from.toLiters / u.toLiters
              return (
